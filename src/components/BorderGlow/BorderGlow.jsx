...
function parseHSL(hslStr) {
  ...
  if (!match) return { h: 180, s: 70, l: 70 };
...
export default function BorderGlow({
  ...
  const handlePointerMove = useCallback((event) => {
    ...
    card.style.setProperty("--glow-x", `${Math.min(Math.max((x / rect.width) * 100, 0), 100).toFixed(3)}%`);
    card.style.setProperty("--glow-y", `${Math.min(Math.max((y / rect.height) * 100, 0), 100).toFixed(3)}%`);
    card.style.setProperty("--edge-proximity", `${(edge * 100).toFixed(3)}`);
    card.style.setProperty("--edge-opacity", `${Math.max((edge * 100 - edgeSensitivity) / (100 - edgeSensitivity), 0).toFixed(3)}`);
